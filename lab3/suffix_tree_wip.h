...
struct SuffixTree {
		...
		for (int i = 0; i < s.size()+1; ++i) {
			...
			while (remainder > 0) {
				...
				if (nodes[active_node].children[active_edge] == 0) { // if node does not have a child beginning with letter
					...
					nodes[active_node].real_children.push_back(active_edge);
					...
					nodes[split_idx].real_children.push_back(uc(c));
					//~ leaves.push_back(next_id);
					++next_id;
					...
					nodes[split_idx].real_children.push_back(node_c);
	...
	//~ std::vector<int> all_leaves(int nodeidx) const{
	...
	//~ void dfs(int idx, std::vector<int> & matches) const {
		//~ int n_children = 0;
		//~ if(idx > 1500000)
		//~ std::cerr << idx << "/"<< nodes.size() << "\n"; 
		//~ for (auto a : nodes[idx].children) {
		...
		//~ }
		//~ if(n_children == 0) {
			//~ matches.push_back(nodes[idx].suffix);
		//~ }
	...
	//NEW
	//~ std::vector<int> _suffix_dffs(int nodeidx) const{
		//~ std::vector<int> vi;
		//~ vi.reserve(1000000);
		//~ std::vector<int> idxs;
		//~ idxs.reserve(1000000);
		//~ int start=0;
		//~ int end=1;
	...
	std::vector<int> _suffix_dfs(int nodeidx) const{
		...
		while (!idxs.empty()) {
			...
			//~ bool found = false;
			int child;
			if(nodes[idx].real_children.empty()) vi.push_back(nodes[idx].suffix); else
			for (auto a : nodes[idx].real_children) {
				child = nodes[idx].children[a];
				//~ if(child!=0){
				//~ found = true;
				idxs.push(a);
					//~ std::cerr << "Found a leaf node! idx " << a << std::endl;
	...
	std::vector<int> match(const std::string &pattern) {
		...
		//VITAL
		//~ auto a = all_leaves(idx);
		auto v = _suffix_dfs(idx);
		return v;
