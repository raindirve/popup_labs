...
struct SuffixTree {
	...
	SuffixTree(const std::string &s_, const int & end_token = 256+36) : s(s_) {
		...
		for (int i = 0; i < s.size()+1; ++i) {
			...
			while (remainder > 0) {
				...
				if (nodes[active_node].children.count(active_edge) == 0) { // if node does not have a child beginning with letter
					...
					int sc = s[nodes[node_idx].begin + active_length];
					if ( sc == c) { // next position matches new char -> advance i
						++active_length;
						if (suffix_link_from != 0) { // set suffix link from
					...
					int node_c = s[nodes[node_idx].begin]; // first letter of node
...
void dprint(SuffixTree &st) {
	...
	while (!q.empty()) {
		...
		for (auto p : n.children) {
			std::cerr << "  edge: " << char(p.first) << " id: " << p.second << "\n";
