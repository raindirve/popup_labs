...
Point<long double> operator+(Point<T> lhs, const Point<U> & rhs) {
	return Point<long double>(lhs.x + rhs.x, lhs.y + rhs.y);
}
...
template<typename T, typename U>
Point<long double> operator*(const Point<T> & lhs, const U & val) {
	return Point<long double>(double(lhs.x) * val, double(lhs.y) * val, 0);
}
...
template<typename T, typename U>
Point<long double> operator*(const U & val, const Point<T> & rhs) {
	return rhs * val;
}

template<typename T>
Point<long double> operator*(const T & val, const Point<T> & rhs) {
	return rhs * val;
}

template<typename T, typename U>
Point<long double> operator/(const Point<T> & lhs, const U & val) {
	return Point<long double>(double(lhs.x) / double(val), double(lhs.y) / double(val), 0);
}
...
template<typename T>
long double area(const Point<T> & p1, const Point<T> & p2, const Point<T> & p3) {
	return long double(cross(p2-p1,p3-p1)) / 2;
}

template<typename T>
long double angle(const Point<T> & p) {
	//~ assert(p.x != 0 || p.y != 0);
	return atan2(p.y,p.x);
}

template<typename T>
long double angle(const Point<T> & p1, const Point<T> & p2) {
...
long double length(const Point<T> & p) {
	return sqrt(longdouble(length2(p)));
}
