...
/**
positive dist to line -> keep inside new polygon
*/
using std::cerr;
using std::endl;
template <typename T>
Polygon<T> cut(const Polygon<T> & poly, const Line<T> & line){
	Polygon<T> rv;
	//~ cerr << "==== CUTTING ALONG " << line << " ==== " << endl;
	//~ cerr << poly << endl;
	
	for(int i = 0; i< poly.n_points(); ++i){
		int inext = (i+1) % poly.n_points();
		//~ cerr << "\tCurrent point: " << poly.points[i] << ", next point: " << poly.points[inext] << endl;
		
		if(dist(line,poly.points[i]) >= 0) { // this point is on the line;
			//~ cerr << "\tAdded point: " << poly.points[i]  << endl;
			rv.add_point(poly.points[i]);
			if(dist(line,poly.points[inext]) < 0 && dist(line,poly.points[i]) > 0) {
				Line<double> ict = intersect(line,Line<T>(poly.points[i],poly.points[inext],true));
				rv.add_point(ict.p1);
				//~ cerr << "\tAdded intersect-in point: " << ict.p1 << endl;
			}			
		} else {
			if(dist(line,poly.points[inext]) > 0) {
				Line<double> ict = intersect(line,Line<T>(poly.points[i],poly.points[inext],true));
				rv.add_point(ict.p1);
				//~ cerr << "\tAdded intersect-out point: " << ict.p1 << endl;
			}	
		}			
	}
	//~ cerr << "==== ==== " << endl;
